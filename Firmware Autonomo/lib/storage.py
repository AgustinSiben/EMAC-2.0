...
class SD:
    ...
    # SD card init mount on /sd
        ...
        if self._status: self.update_file_name_offset()
    ...
    def update_file_name_offset(self)->None:
        self._files_name_offset = len(uos.listdir("/sd/datos"))
        return
...
def read(file_path, length=None) -> str | bytes:
    try:
...
def delete_file_contents(file_path):
    try:
        with open(file_path, 'wb') as f:
            f.write(b'')
        print("File contents successfully deleted.")
    except:
        print("Error deleting file content")
...
def get_free_space(style='absolute') -> float:  # type: ignore #return free space
    ...
    status = uos.statvfs('//')
    if style == 'absolute':
        # Block size/free_block / Megabytes
        return ((status[0]*status[3])/1048576)  # type: ignore
    if style == 'percentage':
        # Free blocks / total blocks
        return (status[3])/(status[2])  # type: ignore
    if style == 'used':
        # block size (total blocks - free blocks)
        return status[0]*(status[2]-status[3])  # type: ignore


def space_of_data() -> int:  # return space of '/data.bin'
    # Si no tiene nada retorna 4 porque es el mínimo que tiene le Mercury
    # Hace referencia a los 4 bytes donde se almacenan esos datos
    # Acá no los uso porque los consulto antes de enviar
    try:
...
def delete_content_from_file(file_name, content_to_delete):
    with open(file_name, 'rb') as file:
        content = file.read()
    content = content.replace(content_to_delete, b'')
