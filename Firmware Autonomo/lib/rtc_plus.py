...
class Rtc_Plus:
    def __init__(self) -> None:
        # -1 autocalculate weekday format rtc
        self._ZERO_AT = (2000, 1, 1, -1, 0, 0, 0, 0)
        self._JULIAN_ZERO = mktime(self.format_rtc_to_time(self._ZERO_AT))
        self._rtc = RTC()
        self._rtc.datetime(self._ZERO_AT)  # Init at 01/01/2000 00:00:00hs

    def set_time(self, time) -> None:
        if type(time) == tuple:
            self._rtc.datetime(time)
        if type(time) == int:
    ...
    def get_time(self) -> tuple:
        return self.format_rtc_to_time(self._rtc.datetime())

    def get_julian_time(self) -> int:
        return self.date_to_julian(self._rtc.datetime())
    ...
    def time_to(self, date) -> int:  # return in secods time to "date"
        ...
        time_now = mktime(self.format_rtc_to_time(self._rtc.datetime()))
        start_time = mktime(date)  # type: ignore
    ...
    @staticmethod
    def julian_to_time(julian) -> list:
